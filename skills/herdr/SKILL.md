...
The upstream `pi-interactive-subagents` extension does not currently provide a native Herdr mux backend. When a user explicitly asks to use Herdr for a Pi subagent, orchestrate it through the Herdr CLI rather than setting `PI_SUBAGENT_MUX=herdr` or using the upstream `subagent` tool directly.

Recommended workflow:

1. Split a sibling pane with `herdr pane split --current --direction right --no-focus`.
2. Parse the returned JSON and use its actual `pane_id`.
3. Rename the pane and start `pi` with `herdr pane run <pane_id> "pi"`.
4. Wait for the child to become `idle`, then send its task with `herdr pane run <pane_id> "<task>"`.
5. Read the result with `herdr pane read <pane_id> --source recent-unwrapped --lines 120`.

This manual path preserves Herdr's pane/status tracking but does not provide the upstream extension's automatic result steering, interruption, or resume behavior. Do not copy or shadow upstream agent definitions solely to add Herdr instructions.
