...
export function formatAnswers(questions: ExtractedQuestion[], answers: string[]): string {
	const parts: string[] = [];
...
export class QnAComponent implements Component {
	...
	render(width: number): string[] {
		...
		for (let index = 0; index < this.questions.length; index++) {
			const answered = (this.answers[index]?.trim() || "").length > 0;
		...
		for (let index = 1; index < editorLines.length - 1; index++) {
			if (index === 1) {
				lines.push(padToWidth(boxLine(answerPrefix + editorLines[index])));
			} else {
